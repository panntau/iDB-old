...
var
    ...
    images = { in: source + (config.images[--config.images.length] == '/' ? config.images + '**/*.*' : config.images + '/**/*.*'),
        out: dest + config.images
    },
    views = { in: source + (config.views[--config.views.length] == '/' ? config.views + '*.jade' : config.views + '/*.jade'),
        out: dest,
        watch: source + (config.views[--config.views.length] == '/' ? config.views + '**/*' : config.views + '/**/*')
    },
    styles = { in: source + config.sass,
    ...
    js = { in: source + (config.jsDir[--config.jsDir.length] == '/' ? config.jsDir + '**/*' : config.jsDir + '/**/*'),
    ...
    vendors = { in: source + (config.vendors[--config.vendors.length] == '/' ? config.vendors + '**/*' : config.vendors + '/**/*'),
...
gulp.task('js', function() {
    if (devBuild) {
        ...
        return gulp.src(js.in)
            ...
            .pipe($.concat(js.filename))
            .pipe($.size({
                title: 'Javascript In Size'
            }))
            .pipe($.stripDebug())
            .pipe($.uglify())
            .pipe($.size({
                title: 'Javascript Out Size'
            }))
            .pipe(gulp.dest(js.out));
    }
});
...
gulp.task('jade', function() {
    ...
    if (!devBuild) {
        ...
        templates = templates
            .pipe($.size({
                title: 'Jade Templates Before Compression'
            }))
            .pipe($.jade())
            .pipe($.size({
                title: 'Jada Templates After Compression'
            }));
    } else {
